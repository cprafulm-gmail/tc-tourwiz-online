...
class UmrahPackageAddOffline extends Form {
  state = {
    data: {
  ...
  setDates = () => {
    let newData = { ...this.state.data };
    let packageInfo = JSON.parse(localStorage.getItem("umrahPackageDetails"));

    if (!packageInfo.startDate) {
      newData.startDate = new Date();
    } else {
      newData.startDate = packageInfo.startDate;
    }

    if (!packageInfo.endDate) {
      newData.endDate = new Date(new Date().getTime() + 1 * 24 * 60 * 60 * 1000);
    } else {
      newData.endDate = packageInfo.endDate;
    }

    if (!packageInfo.departStartDate) {
      newData.departStartDate = new Date();
    } else {
      newData.departStartDate = packageInfo.departStartDate;
    }

    if (!packageInfo.departEndDate) {
      newData.departEndDate = new Date(new Date().getTime() + 1 * 24 * 60 * 60 * 1000);
    } else {
      newData.departEndDate = packageInfo.departEndDate;
    }

    if (!packageInfo.returnStartDate) {
      newData.returnStartDate = new Date();
    } else {
      newData.returnStartDate = packageInfo.returnStartDate;
    }

    if (!packageInfo.returnEndDate) {
      newData.returnEndDate = new Date(new Date().getTime() + 1 * 24 * 60 * 60 * 1000);
    } else {
      newData.returnEndDate = packageInfo.returnEndDate;
    }
  ...
  render() {
    ...
    const packageInfo = JSON.parse(localStorage.getItem("umrahPackageDetails"));

    const count = packageInfo.duration && Math.ceil(packageInfo.duration);
        ...
        <div className="row">
                ...
                <div className="row">
                  ...
                  <div className="col-lg-3">
                    <div className="row">
                      ...
                      <div className="col-lg-4">
                        {this.renderInput("departDuration", "Duration")}
                      </div>
                  ...
                  <div className="row">
                    ...
                    <div className="col-lg-3">
                      <div className="row">
                        ...
                        <div className="col-lg-4">
                          {this.renderInput("returnDuration", "Duration")}
                        </div>
          ...
          <div className="col-lg-2">
            <div className="form-group">
              ...
              >
                Add {type === "Quotation" && business !== "custom" && "Offline"} {business}
...
export default UmrahPackageAddOffline;
