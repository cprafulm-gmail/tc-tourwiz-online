...
class ResultItemAirFareRule extends Component {
  constructor(props) {
    super(props);
    this.state = {
      fareRules: null,
  ...
  getFareRules = (token, callback) => {
    ...
    apiRequester(
      ...
      function (data) {
        ...
        // if (this.props.handlefareRules)
        //   this.props.handlefareRules(data.response);
      }.bind(this)
    );
  };

  handleChangeTabs = (flightOptionToken, tabName) => {
    if (this.props.handleChangeTabs)
      this.props.handleChangeTabs(flightOptionToken, tabName);
    else {
      this.setState({
        activeTab: tabName
      });
  ...
  render() {
    let activeTab = this.props.handleChangeTabs
    ...
    let onwordTripName =
      ...
      "-" +
      this.props.items[0].item[this.props.items[0].item.length - 1].locationInfo
        .toLocation.id;
    let returnTripName =
      this.props.tripType === "roundtrip"
        ? this.props.items[1].item[0].locationInfo.fromLocation.id +
        "-" +
        this.props.items[1].item[this.props.items[1].item.length - 1]
          .locationInfo.toLocation.id
        : "";
    let fareHTML = () => {
        ...
        let data =
          activeTab === "price" || activeTab === "onword"
            ...
            : this.state.fareRules.slice(
              ...
              this.props.items[0].item.length +
              this.props.items[1].item.length
            );
    ...
    return (
      <div
        className={
          "col-lg-" + (this.props.fromPage === "result-item-row" ? "8" : "12")
        }
      >
        <div
          className={
            this.props.fromPage === "result-item-row" ? "mt-4 mb-4" : ""
          }
        >
          ...
          {this.state.fareRules && (
            ...
            >
              <button
                ...
                onClick={() =>
                  this.handleChangeTabs(this.props.flightOptionToken, "onword")
                }
              >
                {onwordTripName}
              </button>
              {this.props.tripType === "roundtrip" &&
                <button
                  ...
                  onClick={() =>
                    this.handleChangeTabs(this.props.flightOptionToken, "return")
                  }
                >
                  {returnTripName}
                </button>
...
export default ResultItemAirFareRule;
