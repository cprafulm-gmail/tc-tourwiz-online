...
const ViewTransportationDetails = props => {
    return (
        <React.Fragment>
            <div className="card shadow-sm mb-3">
                <div className="card-header">
                    <h5 className="m-0 p-0">
                        {Trans("_transportationDetails")}
                    </h5>
                </div>
                    ...
                    <ul className="list-unstyled p-0 m-0">
                        <li className="row">
                            <h6 className="col-lg-9">
                                {props.vendors.find(x => x.type === "company").item.name
                                    ? props.vendors.find(x => x.type === "company").item.name
                                    : props.vendors.find(x => x.type === "company").item.tpExtension.find(y => y.key === "description").value}
                            </h6>
                        </li>
                        <li className="row">
                            <label className="col-lg-3">
                                {Trans("_widgettransportationRoute") + " : "}
                            </label>
                            <b className="col-lg-9">{props.stopInfo.find(x => x.type === "routeDetails").item[0].name}</b>
                        </li>
                        <li className="row">
                            <label className="col-lg-3">
                                {Trans("_viewPhone") + " : "}
                            </label>
                            <b className="col-lg-9">{props.vendors.find(x => x.type === "operator").item.contactInformation.phoneNumber}</b>
                        </li>
                        <li className="row">
                            <label className="col-lg-3">
                                {Trans("_email") + " : "}
                            </label>
                            <b className="col-lg-9">{props.vendors.find(x => x.type === "operator").item.contactInformation.email}</b>
                        </li>
                    </ul>
                </div>
            </div>
...
export default ViewTransportationDetails;
