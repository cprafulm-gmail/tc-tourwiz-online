...
const PackageAppliedFilter = (props) => {
  return (
    <div>
      ...
      {Object.keys(filterSettings).map((item, index) => {
        ...
        if (item === "dateMode" && props.filterData[item] === "specific-date") {
          ...
          item === "dateMode" &&
          props.filterData[item] === "between-dates"
        ) {
          value +=
            " - " +
            props.filterData["fromDate"] +
            " to " +
            props.filterData["toDate"];
        } else if (
          item === "dateMode" &&
          props.filterData[item] === "specific-month"
        ) {
          value +=
            " - " +
            monthLookup.find(
              (x) => x.value === props.filterData["specificmonth"]
            ).name;
        }
        //Custom Code Ends
        ...
        return !filterSetting ? (
          ...
          <div className="btn-group m-1" key={index}>
            <span className="badge badge-info badge-info-custom">
              {filterSetting.title ?? ""} <b>{value}</b>
              {(filterSetting.isNonRemoveable === undefined ||
                filterSetting.isNonRemoveable !== true) && (
                  <small
                    className="m-1"
...
const filterSettings = {
  packagename: { title: "Package Name" },
  customername: { title: "Customer Name" },
  email: { title: "Email" },
  phone: { title: "Phone" },
  rating: { title: "Star Rating" },
  countryName: { title: "Country" },
  stateName: { title: "State" },
  cityName: { title: "City" },
  searchBy: {
    title: "Search By",
    isNonRemoveable: true,
    lookup: searchByItineraryLookup,
  },
  dateMode: { isNonRemoveable: true, lookup: dateModeLookup },
};

export default PackageAppliedFilter;
