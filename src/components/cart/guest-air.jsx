...
class GuestAir extends Form {
  ...
  getFiles(files) {
    ...
    if (
      !files.type.includes("pdf/") || !files.type.includes("image/")
    ) {
      this.setState({ uploadDocValidationImage: "Invalid file selected." });
      return;
    }
    else if (files.file.size > 1024000) {
  ...
  validate = () => {
    ...
    // if (this.props.isPaperRateBooking && this.props.isPaperRateBooking === true) {
    //   if (!this.validateFormData(data.eticketno, "require"))
    //     errors.eticketno = "E-Ticket Number required";
    //   else if (!this.validateFormData(data.eticketno, "alpha_numeric"))
    //     errors.eticketno = Trans("Enter valid E-Ticket Number");
    //   else if (
    //     !this.validateFormData(data.eticketno, "length", {
    //       min: 5,
    //       max: 20,
    //     })
    //   )
    //     errors.eticketno = Trans("E-Ticket Number must be between 5 and 20 characters long");
    // }
  ...
  setUserInfo = (traveler) => {
    ...
    if (passportExpirationDate !== "") {
      let passportExpirationUserDate = moment(passportExpirationDate);
  ...
  render() {
    ...
    let isPaperRateInternationalBooking = this.props.isPaperRateBooking && this.props.isInternational;
    return (
      <div className="row">
        ...
        <div className="col-lg-3">
          {this.renderSelect(
            "documentType",
            (isPaperRateInternationalBooking || !this.props.isPaperRateBooking) ? Trans("_lblDocumentTypeWithStar") : Trans("_lblDocumentType"),
            documentTypes
          )}
        </div>
        ...
        <div className="col-lg-3">
          {this.renderSelect(
            "issuingCountryCode",
            (isPaperRateInternationalBooking || !this.props.isPaperRateBooking) ? Trans("_lblIssuingCountryWithStar") : Trans("_lblIssuingCountry"),
            CountryList,
            "isoCode",
            "name"
          )}
        ...
        <div className="col-lg-12 mb-3">
          {!isMilesCardNotAllowed && flyerCardList.length > 1 && (
            <button
        ...
        {!isMilesCardNotAllowed && isFrequentFlyer && (
          <div className="col-lg-12 mb-3">
