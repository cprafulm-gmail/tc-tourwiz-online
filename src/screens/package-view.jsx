...
class ItineraryEmailView extends Component {
    ...
    html2canvas(document.querySelector("#preview"), {
      ...
      .then((canvas) => {
        ...
        if (type === "print") {
          ...
          document.getElementById('pdf-preview').outerHTML = document.getElementById('pdf-preview').outerHTML;
  ...
  render() {
    ...
    let supplierObj = data.twOthers && JSON.parse(data.twOthers);
    let totalPrice = 0;
    offlineItems &&
      offlineItems.map(
        (item) =>
    ...
    var logo = (this?.props?.userInfo?.provider?.logo?.url
      ?? (portalType === 'B2C' && this?.props?.userInfo?.profilePicture?.url
        ? this?.props?.userInfo?.profilePicture?.url : "")
      ?? "");
    var contactName = this?.props?.userInfo?.provider?.name
      ?? (portalType === 'B2C' && this?.props?.userInfo?.loggedInUserDisplayName
        ? this?.props?.userInfo?.loggedInUserDisplayName : "")
      ?? ''
    return (
      <>
        <div id="preview">
                  ...
                  <div className="container">
                    ...
                    {data.price > 0 && <div className="text-center text-white mt-2">
                      ...
                      <div className="text-white mt-0 text-center">
                        {parseInt(supplierObj?.Adults) > 0 && <small className="mr-3">
                          Adults:{" "}
                          <b>{supplierObj.Adults}
                          </b>
                        </small>}
                        {parseInt(supplierObj?.Child) > 0 && <small className="mr-3">
                          Children:{" "}
                          <b>{supplierObj.Child}
                          </b>
                        </small>}
                        {parseInt(supplierObj?.Infants) > 0 ? <small className="mr-3">
                          Infants:{" "}
                          <b>{supplierObj.Infants}
                          </b>
                        </small> : <span></span>}
                      </div>
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={() => AuthorizeComponentCheck(this.props.userInfo.rolePermissions,
                            "PackageDetails~package-copy-link")
                        ...
                        <button
                          className="btn btn-sm btn-outline-primary text-white border mr-2"
                          onClick={() => AuthorizeComponentCheck(this.props.userInfo.rolePermissions,
                            "PackageDetails~package-download-pdf")
                    ...
                    >
                      {this.props.userInfo && portalType !== 'B2C' && (<button
                        className="btn btn-sm btn-outline-primary text-white border mr-2"
                ...
                <div className="container">
                  <div className="quotation-details mt-3 mb-5">
                    ...
                    <div className="row mt-4">
                      ...
                      {data.hotelDetails && data.hotelDetails != undefined && data.hotelDetails != "" && (
                        <div className="col-lg-12 mt-4 ">
                          <div style={{ background: "rgb(241, 241, 241)", padding: "8px", borderRadius: "4px;" }}><b>Hotel Details</b></div>
                          <div className="pt-3 shadow-sm" style={{ border: "1px solid rgb(241, 241, 241)" }}>
                            <div
                              className={"col-lg-12 pb-3 cms-details-Itinerary"}
                            >
                              <HtmlParser text={decode(data.hotelDetails).replace("<table>", "<table style='width: 100%; border: 1px solid #dee2e6;'>").replaceAll("<td>", "<td style='border-left: 1px solid #dee2e6;'>").replaceAll("<th>", "<th style='border-left: 1px solid #dee2e6;'>")} />
                            </div>
                          </div>
                        </div>
                      )}
                        ...
                        <tr>
                          <td style={{ padding: "16px 0px 8px 0px" }}>
                            {logo ? (
                              <img
                                id="supplierLogo"
                                src={this.getImageCall("supplierLogo", logo)}
        ...
        <iframe id="pdf-iframe" frameborder="0" style={{ display: "none" }}></iframe><div id="pdf-preview">
        </div>
      </>
    );
  }
