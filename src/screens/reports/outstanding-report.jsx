...
class OutstandingReport extends Component {
    ...
    getData = (isExport) => {
        ...
        var reqURL = "tw/reports/outstanding?pagenumber=1&pagesize=0";
            ...
            (data) => {
                ...
                results.map(groupItem => {
                    ...
                    dataValue.push({
                        ...
                        type: "footer",
                        destination: "Total :",
                        baseAmount: groupItem.totalTotalBaseAmount,
    ...
    getExcelReport() {
        let exportData = this.state.exportData.map((item) => {
            ...
            return {
                ...
                "Paid Amount": Amount({ amount: item.paidAmount, currencyCode: item.currencyCode }),
                "Pending Amount": Amount({ amount: item.pandingAmount, currencyCode: item.currencyCode }),
                "Payment Followup Date": item.type === "data" ? (item.pandingAmount != 0 ? DateComp({ date: item.paymentDueDate, format: Global.getEnvironmetKeyValue("DisplayDateFormate") }) : "--") : "",
            }
        });
        ...
        const workbook = {
            ...
            Sheets: {
                'Outstanding Report': workbook1
            }
        };
        this.setState({ exportData: [], reportLoading: false })
        return XLSX.writeFile(workbook, `OutstandingReport.xlsx`);
    }
                                ...
                                <div className="row">
                                    <div className="border pt-2 pl-3 pr-3 shadow-sm col-12 mb-3">
                                        ...
                                        <div className="mb-3 mt-1 float-left">
                                            ...
                                            >
                                                <option value="customer">Outstanding Report - Customer Wise</option>
                                                <option value="product">Outstanding Report - Product Wise</option>
                                                <option value="salesteamwise">Outstanding Report - Sales Team Wise</option>
                                            </select>
                                        </div>
                                        <div className="mb-3 mt-1 float-right">
                                            <AuthorizeComponent title="OutestandingReport~report-export-outstanding" type="button" rolepermissions={this.props.userInfo.rolePermissions}>
                                                {!isBtnLoading &&
                                                    <React.Fragment>
                                                        {!this.state.reportLoading &&
                                                            <button
                                                                className="btn btn-sm btn-primary pull-right"
                                                                onClick={() => { AuthorizeComponentCheck(this.props.userInfo.rolePermissions, "OutestandingReport~report-export-outstanding") ? this.getData(true) : this.setState({ isshowauthorizepopup: true }) }}
                                                            >
                                                                Export
                                                            </button>
                                                        }
                                                        ...
                                                        return (item.type === "header" ?
                                                            ...
                                                            : item.type === "data" ?
                                                                <tr key={index}>
                                                                    ...
                                                                    <td>{item.pandingAmount != 0 ? <DateComp date={item.paymentDueDate} format={Global.getEnvironmetKeyValue("DisplayDateFormate")} /> : "--"}</td>
                                                                </tr>
                                                                : item.type === "footer" ?
