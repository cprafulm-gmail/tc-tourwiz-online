...
class SupplierReport extends Component {
    ...
    getData = (isExport) => {
        ...
        var reqURL = "tw/reports/supplier?pagenumber=1&pagesize=0";
        ...
        apiRequester_unified_api(
            ...
            (data) => {
                ...
                results.map(groupItem => {
                    ...
                    if (this.state.filter.groupBy === "supplier") {
                        dataValue.push({
                            ...
                            totalNumberOfCancellations: groupItem.totalTotalNumberOfCancellations,
                            totalRefundsMade: groupItem.totalTotalRefundsMade,
                            totalCommissionsOwed: groupItem.totalTotalCommissionsOwed,
                ...
                //         totalNumberOfCancellations: dataValue.reduce((sum, item) => sum + (item.type === "data" && item.totalNumberOfCancellations ? item.totalNumberOfCancellations : 0), 0),
                //         totalRefundsMade: dataValue.reduce((sum, item) => sum + (item.type === "data" && item.totalRefundsMade ? item.totalRefundsMade : 0), 0),
                //         totalCommissionsOwed: dataValue.reduce((sum, item) => sum + (item.type === "data" && item.totalCommissionsOwed ? item.totalCommissionsOwed : 0), 0),
    ...
    getExcelReport() {
        let exportData = this.state.exportData.map((item) => {
            ...
            return {
                ...
                "Name of Customer": item.nameOfCustomer,
                "Number of participants": item.numberOfParticipants,
                "Booking channel": item.bookingChannel,
                "Total Value": Amount({ amount: item.totalValue, currencyCode: item.SupplierCurrency }),
                "Supplier Currency Conversion Rate": item.type == "data" ? parseFloat(item.supplierCurrencyConversionRate).toFixed(5) : "",
                ...
                "Total number of cancellations": item.type == "data" ? 0 : "",
                "Total refunds made": Amount({ amount: item.totalRefundsMade, currencyCode: item.currencyCode }),
                "Total commissions owed": Amount({ amount: item.totalCommissionsOwed, currencyCode: item.currencyCode }),
        ...
        workbook1['!cols'] = [
            ...
            { wpx: 150 }, { wpx: 150 }, { wpx: 50 }, { wpx: 75 }, { wpx: 100 },
            { wpx: 50 }, { wpx: 100 }, { wpx: 100 }, { wpx: 100 }, { wpx: 50 },
            { wpx: 100 }, { wpx: 100 }, { wpx: 100 }, { wpx: 100 }
        ];
        const workbook = {
            SheetNames: ['Supplier Report'],
            Sheets: {
                'Supplier Report': workbook1
            }
        };
        this.setState({ exportData: [], reportLoading: false })
        return XLSX.writeFile(workbook, `SupplierReport.xlsx`);
    }
                                ...
                                <div className="row">
                                    <div className="border pt-2 pl-3 pr-3 shadow-sm col-12 mb-3">
                                        ...
                                        <div className="mb-3 mt-1 float-right">
                                            <AuthorizeComponent title="SupplierReport~report-export-supplier" type="button" rolepermissions={this.props.userInfo.rolePermissions}>
                                                {!isBtnLoading &&
                                                    <React.Fragment>
                                                        {!this.state.reportLoading &&
                                                            <button
                                                                className="btn btn-sm btn-primary pull-right"
                                                                onClick={() => { AuthorizeComponentCheck(this.props.userInfo.rolePermissions, "SupplierReport~report-export-supplier") ? this.getData(true) : this.setState({ isshowauthorizepopup: true }) }}
                                                            >Export</button>
                                                        }
                                                        {this.state.reportLoading &&
                                                            <button
                                        ...
                                        <table className="table border table-column-width" id="sheet1">
                                            <thead className="thead-light">
                                                <tr>
                                                    ...
                                                    <th width={table_column_width["Money In"]} colSpan="10">Money In</th>
                                                    <th width={table_column_width["Money Out"]} colSpan="5">Money Out</th>
                                                        ...
                                                        return (item.type === "header" ?
                                                            ...
                                                            : item.type === "data" ?
                                                                <tr key={index}>
                                                                    ...
                                                                    <td style={{ "word-break": "break-word" }}>{item.bookingNumber}</td>
                                                                    <td style={{ "word-break": "break-word" }}>{item.nameOfCustomer}</td>
                                                                    <td>{item.numberOfParticipants}</td>
                                                                    <td style={{ "word-break": "break-word" }}>{item.bookingChannel}</td>
                                                                    ...
                                                                    <td>{item.totalNumberOfCancellations}</td>
                                                                    <td><Amount amount={item.totalRefundsMade} /></td>
                                                                    <td><Amount amount={item.totalCommissionsOwed} /></td>
                                                                ...
                                                                : item.type === "footer" ?
                                                                    <tr key={index} className="tbody-group-footer">
                                                                        ...
                                                                        <th><Amount amount={item.totalCommissionsOwed} /></th>
